...
raw_data = pd.read_csv('data/Wa_v5.csv', delimiter=',')
areas = ['AUS']
...
required_X_columns.append('Corn_Area')
required_X_columns.append('Covid')
required_y_columns.append('Corn_Production')
...
model = tf.keras.models.Sequential([
    ...
    layers.Dropout(0.1),
    layers.Dense(128, kernel_initializer='random_normal'),
    layers.BatchNormalization(),
    layers.Activation('relu'),
    layers.Dropout(0.1),
    layers.Dense(1),
...
check_points_path = 'checkpoints_corn_yield/model.ckpt'
y = y*1000
...
# history = model.fit(x=X, y=y.values, batch_size=5, epochs=2000, validation_split=0.01,
